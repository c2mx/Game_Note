...
Window_Help.prototype.initialize = function(numLines) {
    var width = Graphics.boxWidth-150;//宽度调整[xiaorui]
    var height = this.fittingHeight(numLines || 1);//帮助高度[xiaorui]
    Window_Base.prototype.initialize.call(this, 150, 0, width, height);//[xiaorui]
    this._text = '';
};
...
Window_ItemCategory.prototype.makeCommandList = function() {
    ...
    this.addCommand(TextManager.cancel, 'cancel');//添加取消[xiaorui]
};
...
Window_ItemList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x, y-2, width, height);//[xiaorui]
    this._category = 'none';
    this._data = [];
};
...
Window_SkillList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x+70, y+45, width-140, height-100);//菜单[xiaorui]
    this._actor = null;
    this._stypeId = 0;
    this._data = [];
};
...
Window_EquipStatus.prototype.refresh = function() {
    this.contents.clear();
    if (this._actor) {
        this.drawActorName(this._actor, this.textPadding()+25, 0);//人物姓名[xiaorui]
        for (var i = 0; i < 6; i++) {
...
Window_EquipStatus.prototype.drawItem = function(x, y, paramId) {
    this.drawParamName(x + this.textPadding()+25, y, paramId);
    if (this._actor) {
...
Window_EquipSlot.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x-20, y-20, width+30, height+10);//[xiaorui]
    this._actor = null;
    this.refresh();
};
...
Window_EquipSlot.prototype.drawItem = function(index) {
    if (this._actor) {
        ...
        this.drawItemName(this._actor.equips()[index], rect.x-3, rect.y+2);//rect.x+138[xiaorui]
        this.changePaintOpacity(true);
        var item = this._actor.equips()[index];
        var react_bk = this.itemRect(index);//xiaoruis
        if (item) {
            this.drawEquipBorder(item, react_bk, index);//xiaoruis
        } else {
            this.clearEquipBorder(index);//xiaoruis
        }
    }
};
...
Window_Options.prototype.updatePlacement = function() {
    ...
    this.y = (Graphics.boxHeight - this.height) / 2 - 80;//设置[xiaorui]
};
...
Window_NameEdit.prototype.charWidth = function() {
    ...
    return this.textWidth(text)+15;//名字字间距[xiaorui]
};
...
Window_ActorCommand.prototype.makeCommandList = function() {
    if (this._actor) {
        //this.addAttackCommand();//[xiaorui]
        this.addSkillCommands();
        //this.addGuardCommand();//[xiaorui]
        this.addItemCommand();
    }
};
